...
test("DMN basic operations test", async () => {
    ...
    expect(await processProps.getDmnNameFromInput()).toEqual("Traffic Violation");
    ...
    expect(await (await decisionNavigator.getNodeNames()).sort()).toEqual(DMN_NODES_NAMES.concat("TextAnnotation-1").sort());
    expect(await decisionNavigator.getDmnName()).toEqual("Traffic Violation");
    await decisionNavigator.selectNode("Driver");
    const nodeProps = await sideBar.openProperties();
    expect(await nodeProps.getDmnNameFromInput()).toEqual("Driver");
...
afterEach(async () => {
    await tools.finishTest("DMN_basic_opepration_test");
});
